...
if diff -q $path_list_check_number_nodes/check_$2.netsim $path_list_check_number_nodes/host_$2.netsim; then
    ...
    echo "" 
   echo "check_$2.netsim and host_$2.netsim are not the same. Please view in $1 Server into /opt/ericsson/enmutils/etc/nodes/"
    echo ""
    cat $path_list_check_number_nodes/diff_$2.netsim
fi
exit 0
