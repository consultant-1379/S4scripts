...
do
	...
	sshpass -p 12shroot ssh -q nagios@$IP_param "sudo ssh root@$wkl_vm_ip ls -1v /opt/ericsson/enmutils/etc/nodes/ | grep $u | sed 's/.athtem.eei.ericsson.se-nodes//' | sed 's/$u-//'  > /tmp/check_$u.netsim"
	# Move check.netsim from Server to Nagios on /tmp
	sshpass -p 12shroot ssh -q nagios@$IP_param "sudo cat /tmp/check_$u.netsim" > /tmp/check_$u.netsim
