...
enm_login=$(sshpass -p 12shroot ssh nagios@$lms_ip "sudo curl -k --request POST \"https://$http_url/login\" -d IDToken1=\"administrator\" -d IDToken2=\"TestPassw0rd\" --cookie-jar cookie.txt")
...
pm_files_query=$(sshpass -p 12shroot ssh nagios@$lms_ip "sudo curl -w \"%{http_code}\" -G -s --insecure --request GET --cookie cookie.txt \"https://$http_url/file/v1/files?filter=dataType==PM_STATISTICAL;nodeType==$netype;startRopTimeInOss==${cyear}-${cmonth}-${cday}T${chour}:${cmin}:00\"")
...
if [[ $no_pm_files -eq 10000 ]];then
  while :;do
    ...
    pm_files_query_extended=$(sshpass -p 12shroot ssh nagios@$lms_ip "sudo curl -w \"%{http_code}\" -G -s --insecure --request GET --cookie cookie.txt \"https://$http_url/file/v1/files?filter=dataType==PM_STATISTICAL;nodeType==$netype;startRopTimeInOss==${cyear}-${cmonth}-${cday}T${chour}:${cmin}:00;id=gt=$last_id\"")
