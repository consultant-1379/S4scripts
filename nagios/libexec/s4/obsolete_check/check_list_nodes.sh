...
do
	...
	sshpass -p 12shroot ssh nagios@$IP_param "sudo ssh root@$wkl_vm_ip sudo ls -1v /opt/ericsson/enmutils/etc/nodes/ | grep $u | sed 's/.athtem.eei.ericsson.se-nodes//' | sed 's/$u-//'  > /tmp/check_$u.netsim"
    ...
    if [ $status_check == 0 ]; then
            echo "OK - there are no different nodes types between ENM / NetSim in Management Server ($wkl_vm_ip)"
            echo ""
            exit 0
    fi

	if [ $status_check -ge 10 ]; then
		echo "CRITICAL - some $neType_param type nodes in management server ($wkl_vm_ip) are different ($status_check)"
		echo ""
	...
	if [ $status_check -lt 10 ]; then
		echo "WARNING - some $neType_param type nodes in management server ($wkl_vm_ip) are different ($status_check)"
		echo ""
		for u_check in "${netsim_name[@]}"
		do
		status_if=$(diff -q $path_list_check_number_nodes/check_$u_check.netsim $path_list_check_number_nodes/host_$u_check.netsim | awk '{print $ 5;}') #deve stare dentro al ciclo interno
