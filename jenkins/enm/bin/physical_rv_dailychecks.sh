...
#Determine if env is cloud or physical based on existence of /var/ericsson/ddc_data/config/ddp.txt on LMS (physical). Otherwise this is considered a cloud env.
        ...
        if [ ${env_type} == "cloud" ]
            ...
            ddp_server=`ssh -i /var/tmp/enm_keypair.pem cloud-user@$EMP "ssh -tt -i /var/tmp/enm_keypair.pem cloud-user@${esmon_ip}" "exec sudo -i" grep ddcDataUpload /var/log/cron |tail -1 | grep -oP ddpenm"\d"`
...
versant_connection_pool(){
        ...
        cat ${rv_dailychecks_tmp_file} |sed 's/#012/\n/g'| sed 's/#011/\t/g'| head -1 | sed "s/^/\t\t/g"
        echo ":" | sed "s/^/\t\t/g"
        echo ":" | sed "s/^/\t\t/g"
        cat ${rv_dailychecks_tmp_file} |sed 's/#012/\n/g'| sed 's/#011/\t/g' | tail -1| sed "s/^/\t\t/g"
        truncate -s 0 ${rv_dailychecks_tmp_file}
}
...
rollback_transactions(){
        ...
        zgrep EJBTransactionRolledbackException /net/${ddp_server}/data/stats/tor/LMI_${CLUSTER}/analysis/${DDP_DATE}/enmlogs/*.csv.gz|grep -oP "@.*@JBOSS@ERROR" | sort | uniq -c | sort -nrk1 | sed "s/^/\t/g"
}
...
unprocessed_alarms_sent_northbound(){
        ...
        echo -e "\e[0;32m===================================================================================\e[0m"
        zgrep -c  "@svc-.*-fmalarmprocessing@JBOSS@INFO.*Max retries to process an alarm reached and unprocessed alarm is sent NorthBound" /net/${ddp_server}/data/stats/tor/LMI_${CLUSTER}/analysis/${DDP_DATE}/enmlogs/*csv.gz | sed "s/^/\t/g"
        echo ""
}
...
unprocessed_alarms_waiting_long_time(){
        ...
        echo -e "\e[0;32m=======================================================================================================\e[0m"
        zgrep -c  "@svc-.*-fmalarmprocessing@JBOSS@WARN.**unprocessed alarms as waiting in the buffer for long time with problematic threads" /net/${ddp_server}/data/stats/tor/LMI_${CLUSTER}/analysis/${DDP_DATE}/enmlogs/*csv.gz | sed "s/^/\t/g"
        echo ""
        #zgrep "@svc-.*-fmalarmprocessing@JBOSS@WARN.**unprocessed alarms as waiting in the buffer for long time with problematic threads" /net/${ddp_server}/data/stats/tor/LMI_${CLUSTER}/analysis/${DDP_DATE}/enmlogs/*csv.gz | cut -d" " -f1,7- | sed "s/^/\t/g"
}
...
notification_buffer_corrupted(){
        ...
        echo -e "\e[0;32m================================================================\e[0m"
        zgrep -c  "@svc-.*-mscmce@JBOSS.*Notification Buffer Corrupted" /net/${ddp_server}/data/stats/tor/LMI_${CLUSTER}/analysis/${DDP_DATE}/enmlogs/*csv.gz | sed "s/^/\t/g"
        echo ""
...
sps_AMOS_EntityNotFoundException_errors(){
        ...
        echo -e "\e[0;32m================================================================\e[0m"
        zgrep -c  "@svc-.*-sps@JBOSS@ERROR.*EntityNotFoundException.*AMOS" /net/${ddp_server}/data/stats/tor/LMI_${CLUSTER}/analysis/${DDP_DATE}/enmlogs/*csv.gz | sed "s/^/\t/g"
        echo ""
...
netsim_stopped_nodes(){
        ...
        if [ ${env_type} = cloud ] ; then
            for i in `ls -ltr /opt/ericsson/enmutils/etc/nodes/|grep -oP "ieatnetsimv[0-9]{4}-[0-9]{2}" | sort | uniq`;
            do
            ...
            for i in `ssh -o StrictHostKeyChecking=no ${workload_vm} "ls -ltr /opt/ericsson/enmutils/etc/nodes/|grep -oP "\""ieatnetsimv[0-9]{4}-[0-9]{2}"\"" | sort | uniq"`;
            do
            SIMS_NOT_STARTED=$(ssh -o StrictHostKeyChecking=no netsim@${i} "echo '.show allsimnes' | /netsim/inst/netsim_shell | grep 'not started' | cut -d' ' -f1")
...
shm_job_failures(){
        ...
        echo -e "\e[0;32m==============================\e[0m"
        for i in `${CLI_APP} 'shm status --all' | grep $(date +%d/%m/%Y) | grep FAILED | awk '{print $2}' | sort | uniq`;
        do echo -e "\e[0;32m${i}\e[0m" | sed 's/^/\t/g';
            for j in `${CLI_APP} 'shm status --all' | grep $(date +%d/%m/%Y) | grep "${i}.*FAILED" | sort -k9 | awk '{print $1}'`;
            do echo -e "\e[0;35m${j}\e[0m" | sed 's/^/\t/g';
...
network_product_info(){
        ...
        for i in `${CLI_APP} 'cmedit get * networkelement -ne=RBS' | awk -F "=" '{print $2}' | cut -c1-24 | sort | uniq | egrep -v "^$"`
            do
            echo $i | sed 's/^/\t/g'
        ...
        for i in `${CLI_APP} 'cmedit get * networkelement -ne=ERBS' | awk -F "=" '{print $2}' | cut -c1-24 | sort | uniq | egrep -v "^$"`
            do
            echo $i | sed 's/^/\t/g'
            ${CLI_APP} "cmedit get $i* networkelement.(nodeModelIdentity,neProductVersion,ossModelIdentity)  -t" | egrep -v "neProductVersion|instance|NetworkElement|^\t$" | awk '$1="";{print}' | sort | uniq -c | sort -nrk1 | sed 's/^/\t\t/g'
        done
        ...
        for i in `${CLI_APP} 'cmedit get * networkelement'| awk -F "=" '{print $2}' | cut -c1-11 | sort | uniq | egrep -v "ieatnets|NETSimI|^$" | sort`
            do
            echo $i | sed 's/^/\t/g'
            ${CLI_APP} "cmedit get $i* networkelement.(nodeModelIdentity,neProductVersion,ossModelIdentity)  -t" | egrep -v "neProductVersion|instance|NetworkElement|^\t$" | awk '$1="";{print}' | sort | uniq -c | sort -nrk1 | sed 's/^/\t\t/g'
        done
...
import_job_failures(){
        ...
        echo -e "\e[0;32m=================================\e[0m"
        for i in `${CLI_APP} 'cmedit import -st'| egrep "$(date +%Y-%m-%d)" | cut -f1-5,13- | egrep -i "failed" | awk '{print $1}'`;
        do echo -e "\e[0;32mImport Job Id ${i}\e[0m" | sed 's/^/\t/g';
...
export_job_failures(){
        ...
        echo -e "\e[0;32m=================================\e[0m"
        for i in `${CLI_APP} 'cmedit export -st'| egrep "$(date +%Y-%m-%d)" | cut -f1-5,9-11,13- | awk '$6 > 0 {print}' | awk '{print $1}'`;
            do echo -e "\e[0;32mExport Job Id ${i}\e[0m" | sed 's/^/\t/g';
