...
lookup_host_from_ip() {
  local ipaddr=$1
  local host_name
  host_name=$(nslookup $ipaddr | head -n 1 | grep name | awk '{print $4}' | awk -F'.' '{print $1}')
  echo $host_name
...
get_lms_host() {
  ...
  local ip_valid
  local lms_host
  msip=$(sudo wget -q -O - --no-check-certificate "${CIFWK_URL}/generateTAFHostPropertiesJSON/?clusterId=${cluster_id}&tunnel=true" | awk -F',' '{print $5}' | awk -F':' '{print $2}' | sed -e "s/\"//g" -e "s/ //g")
...
get_workload_host_physical() {
  local wkl_vm_url
  local wkl_vm
  wkl_vm=$(sudo wget -q -O - --no-check-certificate "${CIFWK_URL}/generateTAFHostPropertiesJSON/?clusterId=${cluster_id}&tunnel=true" | grep -oP "^.*workload" | tail -c 34 | awk -F "," '{print $1}' | sed -r 's/"//g' | sed 's/ //g')
...
get_workload_host_cloud() {
  ...
  local ip_valid
  local workload_host
  wklvmip=$(sudo wget -q -O - --no-check-certificate "${CIFWK_URL}/generateTAFHostPropertiesJSON/?clusterId=${cluster_id}&tunnel=true" | awk -F',' '{print $9}' | awk -F'"' '{print $4}')
...
get_all_netsim_hosts() {
  local netsims
  netsims=$(sudo wget -q -O - --no-check-certificate "${CIFWK_URL}/generateTAFHostPropertiesJSON/?clusterId=${cluster_id}&tunnel=true&pretty=true&allNetsims=true" | grep netsim | grep hostname | awk -F"\"" '{print $4}')
