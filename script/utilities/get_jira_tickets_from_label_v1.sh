...
jira_rest_output=$(curl -s -D- -u $user:$password -X GET -H "Content-Type: application/json" "$QUERY_URL" | grep -oP '"https://jira-oss.seli.wh.rnd.internal.ericsson.com/rest/api/2/issue/*[^"]*-*/' | grep -oP 'https://jira-oss.seli.wh.rnd.internal.ericsson.com/rest/api/2/issue/*[^"]*-[^"]*' | sort | uniq)
