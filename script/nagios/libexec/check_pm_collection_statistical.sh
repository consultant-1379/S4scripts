...
warning_value_float=`echo "$tot_number_nodes_pmsuperv * 0.95" | bc -l`
warning_value=${warning_value_float%.*}

critical_value_float=`echo "$tot_number_nodes_pmsuperv * 0.8" | bc -l`
...
enm_login=$(sshpass -p 12shroot ssh nagios@$1 "sudo curl -k --request POST \"https://ieatENM5623-10.athtem.eei.ericsson.se/login\" -d IDToken1=\"administrator\" -d IDToken2=\"TestPassw0rd\" --cookie-jar cookie.txt")

for n in $netypes;do

	pm_files_query=$(sshpass -p 12shroot ssh nagios@$1 "sudo curl -w \"%{http_code}\" -G -s --insecure --request GET --cookie cookie.txt \"https://ieatENM5623-10.athtem.eei.ericsson.se/file/v1/files?filter=dataType==PM_STATISTICAL;nodeType==$n;startRopTimeInOss==${cyear}-${cmonth}-${cday}T${chour}:${cmin}:00\"")
...
#	echo "NUMBER OF PM FILES: $no_pm_files" $n
	...
	if [[ $no_pm_files -eq 10000 ]];then
		...
		no_pm_files_extended=$(sshpass -p 12shroot ssh nagios@$1 "sudo curl -w \"%{http_code}\" -G -s --insecure --request GET --cookie cookie.txt \"https://ieatENM5623-10.athtem.eei.ericsson.se/file/v1/files?filter=dataType==PM_STATISTICAL;nodeType==$n;startRopTimeInOss==${cyear}-${cmonth}-${cday}T${chour}:${cmin}:00;id=gt=$last_id\" | grep -o 'fileLocation' | wc -l")
...
if [[ ("$tot_no_pm_files" -lt "$warning_value") && ("$tot_no_pm_files" -gt "$critical_value") ]];then
        echo "WARNING- NUMBER OF PM FILES ($tot_no_pm_files) IS BETWEEN 80% AND 95% OF NODES WITH PM SUPERVISION ENABLED ($tot_number_nodes_pmsuperv)"
        exit 1
fi

if [[ "$tot_no_pm_files" -lt "$critical_value" ]];then
        echo "CRITICAL- NUMBER OF PM FILES ($tot_no_pm_files) IS LOWER THAN 80% OF NODES WITH PM SUPERVISION ENABLED ($tot_number_nodes_pmsuperv)"
        exit 2
fi

echo "OK- NUMBER OF PM FILES ($tot_no_pm_files) IS HIGHER THAN 95% OF NODES WITH PM SUPERVISION ENABLED ($tot_number_nodes_pmsuperv)"
exit 0
