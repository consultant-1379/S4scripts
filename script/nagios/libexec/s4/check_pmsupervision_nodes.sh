...
if [[ ("$tot_number_nodes_pmsuperv" -lt "$warning_value") && ("$tot_number_nodes_pmsuperv" -gt "$critical_value") ]];then
	echo "WARNING- NUMBER OF NODES WITH PM SUPERVISION ($tot_number_nodes_pmsuperv) IS BETWEEN 80% AND 95% OF ENM NODES ($tot_number_nodes_enm)"
	exit 1	
fi

if [[ "$tot_number_nodes_pmsuperv" -lt "$critical_value" ]];then
        echo "CRITICAL- NUMBER OF NODES WITH PM SUPERVISION ($tot_number_nodes_pmsuperv) IS LOWER THAN 80% OF ENM NODES ($tot_number_nodes_enm)"
        exit 2
fi

echo "OK- NUMBER OF NODES WITH PM SUPERVISION ($tot_number_nodes_pmsuperv) IS HIGHER THAN 95% OF ENM NODES ($tot_number_nodes_enm)"
exit 0
