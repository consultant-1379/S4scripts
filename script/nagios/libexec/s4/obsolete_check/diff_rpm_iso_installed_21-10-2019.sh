...
if [ $(wc -l /usr/local/nagios/libexec/s4/log_diff_rpm_iso_installed/rpm_compare.txt | awk '{print $1}') == 0 ]; then
        echo "OK - RPM are aligned"
        exit 0
fi
if [ $(wc -l /usr/local/nagios/libexec/s4/log_diff_rpm_iso_installed/rpm_compare.txt | awk '{print $1}') != 0 ]; then
        echo "WARNING - $(wc -l /usr/local/nagios/libexec/s4/log_diff_rpm_iso_installed/rpm_compare.txt | awk '{print $1}') RPM are different"
        exit 1
fi
if [ $ISO_check != 'exist' ]; then
        echo "CRITICAL- ISO is not present !!!"
        exit 2
fi
