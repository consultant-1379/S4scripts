...
#	if [[ $no_pm_files -eq 10000 ]];then
		...
		last_id=$(echo $pm_files_query | grep -o -P '"id".{0,10}' | awk -F ":" '{print $2}' | tail -1 | rev | cut -c 2- | rev)
		pm_files_query=$(sshpass -p 12shroot ssh nagios@$1 "sudo curl -w \"%{http_code}\" -G -s --insecure --request GET --cookie cookie.txt \"https://$http_url/file/v1/files?filter=dataType==PM_STATISTICAL;nodeType==$n;startRopTimeInOss==${cyear}-${cmonth}-${cday}T${chour}:${cmin}:00;id=gt=$last_id\"")
...
#		echo "NUMBER OF PM FILES IN THE EXTENDED QUERY: $no_pm_files_extended"	
        no_pm_files=$(echo $pm_files_query | grep -o 'fileLocation' | wc -l)
#        no_pm_files_ethernet=$(echo $pm_files_query | grep -o 'ethernet' | wc -l)
#        no_pm_files_ethsoam=$(echo $pm_files_query | grep -o 'ethsoam' | wc -l)
...
#echo "NUMBER OF PM FILES ETHERNET: $no_pm_files_ethernet
#echo "NUMBER OF PM FILES ETHSOAM: $no_pm_files_ethsoam
...
echo "OK- NUMBER OF PM FILES ($tot_no_pm_files) IS HIGHER THAN 95% OF NODES WITH PM SUPERVISION ENABLED ($tot_number_nodes_pmsuperv*$number_pm_files_by_node=$tot_expected_pm_files)"
exit 0
