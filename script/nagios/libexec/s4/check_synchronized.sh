...
#for n in $netypes;do
        number_nodes_enm=$(sshpass -p 12shroot ssh nagios@$1 "sudo /opt/ericsson/enmutils/bin/cli_app 'cmedit get * NetworkElement.neType==$netype -t' | grep -i $netype | wc -l")
        tot_number_nodes_enm=$((tot_number_nodes_enm + number_nodes_enm))
        eval "number_nodes_${n}"=$number_nodes_enm
#done

#for m in $netypes;do
        number_nodes_syn=$(sshpass -p 12shroot ssh nagios@$1 "sudo /opt/ericsson/enmutils/bin/cli_app 'cmedit get * CmFunction.syncStatus==SYNCHRONIZED --neType=$netype -t' | grep SYNCHRONIZED | wc -l")
...
if [[ "$tot_number_nodes_syn" -eq "$tot_number_nodes_enm" ]];then 
    ...
    echo "OK- ALL $family NODES ARE SYNCHRONIZED ($tot_number_nodes_syn)!"
    exit 0
fi
