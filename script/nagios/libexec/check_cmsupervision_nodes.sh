...
for m in $netypes;do
        number_nodes_cmsuperv=$(sshpass -p 12shroot ssh nagios@$1 "sudo /opt/ericsson/enmutils/bin/cli_app 'cmedit get * CmNodeHeartbeatSupervision.active==true -neType=$m -t'" | grep true | wc -l)
...
if [[ ("$tot_number_nodes_cmsuperv" -lt "$warning_value") && ("$tot_number_nodes_cmsuperv" -gt "$critical_value") ]];then
	echo "WARNING- NUMBER OF NODES WITH CM SUPERVISION ($tot_number_nodes_cmsuperv) IS BETWEEN 80% AND 95% OF ENM NODES ($tot_number_nodes_enm)"
	exit 1	
fi

if [[ "$tot_number_nodes_cmsuperv" -lt "$critical_value" ]];then
        echo "CRITICAL- NUMBER OF NODES WITH CM SUPERVISION ($tot_number_nodes_cmsuperv) IS LOWER THAN 80% OF ENM NODES ($tot_number_nodes_enm)"
        exit 2
fi

echo "OK- NUMBER OF NODES WITH CM SUPERVISION ($tot_number_nodes_cmsuperv) IS HIGHER THAN 95% OF ENM NODES ($tot_number_nodes_enm)"
exit 0
