...
#      echo "URL does not exist: $rpm_url"
      return 1
    fi
...
get_sg_vnf_laf(){
  ...
  local  sg_vnf_laf
  sg_vnf_laf=$(consul members list | awk -F"-" '{print $3}')
  echo $sg_vnf_laf
}
...
check_sg_name_cloud() {
  ...
  local sgs_vnf_laf
  sg_name=$1
  sgs_vnf_laf=$(get_sg_vnf_laf)

  for sg_vnf_laf in $sgs_vnf_laf; do
    if [[ "$sg_vnf_laf" == "$sg_name" ]]; then
      return 0  
    fi
  done
  return 1
...
message_log() {
  ...
  echo $(date "+%F %T.%3N")" $message_severity  $message_txt"
}
