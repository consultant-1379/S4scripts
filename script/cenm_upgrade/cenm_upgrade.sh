...
deploy_chart(){
  ...
  echo "nohUP helm upgrade $chart_label --values $int_values_yaml_file $chart_file --namespace $namespace --wait --timeout ${chart_timeout}s > ${csar_path}/${chart_label}_upgrade_logs 2>&1 &"
  counter=0
