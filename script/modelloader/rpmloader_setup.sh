...
SCRIPT_DIRECTORY='/var/tmp/myscripts/model_loader_lib_version'

[[ $# -eq 0 ]] && usage

#while getopts "r:s:j:m:o:" option
while getopts "r:j:m:u:" option
do
  case "${option}"
  in
...
#ssh -q root@$MS_IP "cd $SCRIPT_DIRECTORY;./rpmloader.sh -r \"$RPM_URL_LIST\" -s \"$SG_LIST\" -j $JIRA_ID -o $OFFLINE_MODE"
ssh -q root@$MS_IP "cd $SCRIPT_DIRECTORY;./rpmloader.sh -r \"$RPM_URL_LIST\" -j $JIRA_ID -u $ENM_SERVICES_DIR"
