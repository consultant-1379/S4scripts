...
netsim_get_sims() {
local sims
sims=$(sshpass -p netsim ssh -q -o StrictHostKeyChecking=no netsim@$netsim << EOF1	
...
local sim=$1 nodes
nodes=$(sshpass -p netsim ssh -q -o StrictHostKeyChecking=no netsim@$netsim << EOF1
for i in 1;do
echo ".open $sim" 
echo '.selectallsimne'
...
celltrace_nodes=$(sshpass -p netsim ssh -q -o StrictHostKeyChecking=no root@$workload_vm "/opt/ericsson/enmutils/bin/cli_app 'cmedit get * enodebfunction -ne=RadioNode -t' | awk '{print \$1}' | sort | grep -v 'NodeId\|SubNetwork'")
...
for sim in $sims;do
  ...
  pms_nodes=$(sshpass -p netsim ssh -q -o StrictHostKeyChecking=no netsim@$netsim "ls /pms_tmpfs/*$pms_dir")
...
#    r_cmd=$(sshpass -p netsim ssh -q -o StrictHostKeyChecking=no netsim@$netsim "ls /pms_tmpfs/*$pms_dir/$node/c/pm_data/A20240119.2000-2015:1.xml* 1>/dev/null 2>/dev/null;echo \$?")
    if [[ $pmtype == *"PM_STATISTICAL"* ]];then
      if [[ $node == *"PCG"* ]];then
        stat_files_cmd=$(sshpass -p netsim ssh -q -o StrictHostKeyChecking=no netsim@$netsim "ls /pms_tmpfs/*$pms_dir/$node/eric-pmbr-rop-file-store/$pm_stats_file 2>&1")
      else
        stat_files_cmd=$(sshpass -p netsim ssh -q -o StrictHostKeyChecking=no netsim@$netsim "ls /pms_tmpfs/*$pms_dir/$node/c/pm_data/$pm_stats_file 2>&1")
      fi
...
#    if [[ $pmtype == *"PM_CELLTRACE"* ]] && [[ $node == *$celltrace_nodes* ]];then
     if [[ $pmtype == *"PM_CELLTRACE"* ]] && grep -q $node enm_celltrace_nodes.txt;then
      cell_dul_files_cmd=$(sshpass -p netsim ssh -q -o StrictHostKeyChecking=no netsim@$netsim "ls /pms_tmpfs/*$pms_dir/$node/c/pm_data/$pm_cell_dul_file 2>&1")
