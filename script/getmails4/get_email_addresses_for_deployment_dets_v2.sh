...
dets_tickets=$(curl -s -u MBTGWBYEBQ:UADyaxhCDYekTcge3zHck8km -X GET -H "Content-Type: application/json" "$QUERY_URL" | jq -r '.issues[].key')
...
#  echo $dets_tickets
  ...
  for dets_ticket in $dets_tickets; do
    ...
    emails=$(curl -s -D- -u MBTGWBYEBQ:UADyaxhCDYekTcge3zHck8km -X GET -H "Content-Type: application/json" "$QUERY_URL" | grep -oP '"emailAddress": *"[^"]*"' | sort | uniq)
...
#  echo $email_list
  ...
  if [[ $watchers = *"true"* ]];then
    ...
    emails_watchers=$(curl -s -D- -u MBTGWBYEBQ:UADyaxhCDYekTcge3zHck8km -X GET -H "Content-Type: application/json" "$QUERY_URL" | grep -oP '"emailAddress": *"[^"]*"' | sort | uniq)
