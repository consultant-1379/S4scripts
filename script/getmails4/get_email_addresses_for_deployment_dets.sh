...
#   QUERY_URL="https://jira-oss.seli.wh.rnd.internal.ericsson.com:443/rest/api/2/search?jql=project=DETS%20AND%20component='Team%20Grifone'%20AND%20Environment~'$CLUSTERID'%20AND%20Status%20not%20in%20(Closed,Resolved)%20AND%20'Type%20of%20Server%20Access'%20in%20(Shared,%20'Exclusive',%20'Exclusive%20evening',%20'Exclusive%20weekend')"

 QUERY_URL="https://jira-oss.seli.wh.rnd.internal.ericsson.com:443/rest/api/2/search?jql=project=DETS%20AND%20component='Team%20Grifone'%20AND%20Environment~'$CLUSTERID'%20AND%20Status%20not%20in%20(Closed,Resolved)%20AND%20'cf\[25604\]'%20in%20(Shared,%20'Exclusive',%20'Exclusive%20evening',%20'Exclusive%20weekend')"
...
dets_tickets=$(sudo curl -s -u S4_Team:S4_Team -X GET -H "Content-Type: application/json" "$QUERY_URL" | jq -r '.issues[].key')
...
#  echo $dets_tickets
  ...
  for dets_ticket in $dets_tickets; do
    ...
    QUERY_URL="https://jira-oss.seli.wh.rnd.internal.ericsson.com/rest/api/2/issue/$dets_ticket"
    emails=$(sudo curl -s -D- -u S4_Team:S4_Team -X GET -H "Content-Type: application/json" "$QUERY_URL" | grep -oP '"emailAddress": *"[^"]*"' | sort | uniq)
...
#  echo $email_list
  ...
  if [[ $watchers = *"true"* ]];then
    QUERY_URL="https://jira-oss.seli.wh.rnd.internal.ericsson.com/rest/api/2/issue/$dets_ticket/watchers"
    emails_watchers=$(sudo curl -s -D- -u S4_Team:S4_Team -X GET -H "Content-Type: application/json" "$QUERY_URL" | grep -oP '"emailAddress": *"[^"]*"' | sort | uniq)
