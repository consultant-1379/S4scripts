...
nas_hostname=$(execute_cmd_nas $nas_user $nas_password $nas_ip "hostname")
echo $nas_hostname
}
...
get_va_version(){
local nas_va_version nas_config_info nas_user=$1 nas_password=$2 nas_ip=$3
nas_va_version=$(execute_cmd_nas $nas_user $nas_password $nas_ip "/usr/bin/cat /opt/VRTSnas/conf/banner" | grep Veritas | awk '{print $6}')
echo $nas_va_version

}

get_va_bonding(){
local nas_network_info nas_user=$1 nas_password=$2 nas_ip=$3
nas_network_info=$(execute_cmd_nas $nas_user $nas_password $nas_ip 'for iface in $(ls /sys/class/net);do echo "Interface:$iface";ethtool $iface 2>/dev/null | grep -i speed;done')
echo "$nas_network_info"
}
...
checkNasDiskInfo(){
local nas_user=$1 nas_password=$2 nas_ip=$3 nas_vip_state nas_disk_details 
nas_disk_details=$(execute_cmd_nas $nas_user $nas_password $nas_ip "/opt/VRTSnas/clish/bin/clish -u master -c 'storage disk list detail'")
echo "$nas_disk_details"

}

getNasWWNPDetails(){
local nas_user=$1 nas_password=$2 nas_ip=$3 nas_host=$4 nas_vip_state nas_wwnp_details
nas_wwnp_details=$(execute_cmd_nas $nas_user $nas_password $nas_ip "/opt/VRTSnas/clish/bin/clish -u master -c 'storage hba $nas_host'")
echo "$nas_wwnp_details"
}
