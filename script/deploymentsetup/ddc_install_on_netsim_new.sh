...
ssh root@$NETSIM date
[[ $? -ne 0 ]] && (echo "Problem with password-less access to $NETSIM ...cannot continue"; exit 1)

echo
echo "### Installing sysstat"
ssh -q root@$NETSIM "if [[ -f /etc/system-release ]];then yum -y install sysstat;fi"
ssh -q root@$NETSIM "if [[ -f /etc/SuSE-release ]];then zypper --non-interactive install sysstat;fi"
...
ssh root@$NETSIM 'echo "* * * * *    root [ -x /usr/lib64/sa/sa1 ] && exec /usr/lib64/sa/sa1 -S ALL 1 1" > /etc/cron.d/sysstat'
ssh root@$NETSIM 'echo "*/5 * * * *    root [ -x /usr/lib64/sa/sa2 ] && exec /usr/lib64/sa/sa2 -S ALL 1 1" >>/etc/cron.d/sysstat'
ssh root@$NETSIM "cat /etc/cron.d/sysstat"
...
scp $DDC_RPM_FILE root@$NETSIM:/var/tmp/

echo
OLD_DDC=$(ssh root@$NETSIM rpm -qa | grep ddc_CXP)
echo "uninstalling" ${OLD_DDC}
ssh root@$NETSIM "rpm -e ${OLD_DDC}"
echo "### Installing DDC package on $NETSIM"
INSTALLED=$(ssh root@$NETSIM rpm -qa | egrep ddc)
[[ -z $INSTALLED ]] && OPTION="i" || OPTION="U"
echo OPTION=$OPTION
ssh root@$NETSIM "rpm -${OPTION}vh /var/tmp/ERICddccore*${VERSION}.rpm --nodeps" ;
echo "Turn on ddc service"
ssh root@$NETSIM "chkconfig ddc on"
...
ssh root@$NETSIM  service ddc status

echo
echo "### Checking installed DDC version"
ssh root@$NETSIM  rpm -qa | egrep ddc
