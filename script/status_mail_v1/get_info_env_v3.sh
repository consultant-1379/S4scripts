...
get_enm_iso() {
  if [ "$deployment_type" == "Physical" ];then
	...
	IP=$(ssh -q root@$wkl_vm 'env|grep EMP| cut -d'=' -f 2' )
	
	enm_iso_version=$(ssh -q -t root@$wkl_vm "ssh -q -t -o StrictHostKeyChecking=no -i /var/tmp/enm_keypair.pem cloud-user@${IP} \"sudo consul kv get enm/deployment/enm_version | cut -d' ' -f 5\"")
	if [ -z "$enm_iso_version" ];then
		enm_iso_version="N/A"
	fi
...
get_nss() {
  ...
  nss_version=$(ssh -q root@$netsim 'cat /netsim/simdepContents/nssProductSetVersion' | awk -F "=" '{print $2}')
...
get_genstats() {
  ...
  for netsim in $netsims;do
  genstats_version=$(ssh -q root@$netsim "/netsim_users/pms/bin/genStatsRPMVersion.sh" | awk -F "-" '{print $2}')  
...
get_nrm() {
  touch nrm_info.txt
  for netsim in $netsims;do	
    nrm_info=$(ssh -q root@$netsim 'cat /netsim/simdepContents/NRMDetails')
