...
getNodesFromSed(){
  ...
  nodes=$(grep 'hostname=' $SED_FILE | grep $node_type | awk -F '_' '{print $1"_"$2}' | sort | uniq)
  echo $nodes
...
sshpass -p $nas_password ssh -q -A -t -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $nas_user@$nas_ip "$cmd"
}
